...
var odbcDsns = Loading.LoadRecords<OdbcDsnRecord>(Path.Combine(rawRoot, "odbc_dsn_tests.json"), json, x => !string.IsNullOrWhiteSpace(x.Server) && !string.IsNullOrWhiteSpace(x.DsnName) && !string.IsNullOrWhiteSpace(x.DriverName) && !string.IsNullOrWhiteSpace(x.Type) && !string.IsNullOrWhiteSpace(x.Architecture), "ODBC DSN");
var certificates = Loading.LoadRecords<CertificateRecord>(Path.Combine(rawRoot, "certificates.json"), json, x => !string.IsNullOrWhiteSpace(x.Server) && !string.IsNullOrWhiteSpace(x.Subject) && !string.IsNullOrWhiteSpace(x.Thumbprint) && !string.IsNullOrWhiteSpace(x.NotAfter) && !string.IsNullOrWhiteSpace(x.Store), "certificate");
var sqlAgentJobs = Loading.LoadRecords<SqlAgentJobRecord>(Path.Combine(rawRoot, "sql_agent_jobs.json"), json, x => !string.IsNullOrWhiteSpace(x.Server) && !string.IsNullOrWhiteSpace(x.Instance) && !string.IsNullOrWhiteSpace(x.JobName), "SQL Agent job");
var ssrsSubscriptions = Loading.LoadRecords<SsrsSubscriptionRecord>(Path.Combine(rawRoot, "ssrs_subscriptions.json"), json, x => !string.IsNullOrWhiteSpace(x.Server) && !string.IsNullOrWhiteSpace(x.Instance) && !string.IsNullOrWhiteSpace(x.ReportPath) && !string.IsNullOrWhiteSpace(x.SubscriptionDescription), "SSRS subscription");
